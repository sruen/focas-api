...
def _gap(strength: StrengthContext) -> float:
...
def _pressure_value(label: str | None) -> float:
    return float(PRESSURE_ORDER.get(label or "弱", 0))
...
def _pressures(
    *,
    ...
    gap = _gap(strength)
    scores = {direction: _pressure_value(pmap.get(direction).strength if pmap.get(direction) else None) for direction in DIRECTIONS}
    if gap >= 1.0:
        scores["主胜"] += 0.5
        scores["客胜"] -= 0.25
    elif gap <= -1.0:
    ...
    if match.neutral_venue or match.real_home_away is False:
        scores["主胜"] -= 0.25
        scores["平局"] += 0.35
    text = " ".join(str(x) for x in (match.stage, match.match_type, match.competition) if x)
    if any(key in text for key in ("友谊", "热身", "淘汰", "单回合", "决赛", "半决赛")):
        scores["平局"] += 0.25
...
def _first_eye_direction(pulls: list[NaturalPull], pressures: dict[str, str]) -> str | None:
    ...
    if explicit:
        return max(explicit, key=lambda direction: PRESSURE_ORDER.get(pressures.get(direction, "弱"), 0))
    ranked = sorted(DIRECTIONS, key=lambda direction: PRESSURE_ORDER.get(pressures.get(direction, "弱"), 0), reverse=True)
    return ranked[0] if ranked and PRESSURE_ORDER.get(pressures[ranked[0]], 0) > 0 else None


def _distribution_type(*, match: MatchContext, strength: StrengthContext, pressures: dict[str, str]) -> str:
    ...
    if match.neutral_venue or pressures["平局"] == "中":
        return "缓冲分布"
    if gap >= 0.5 and PRESSURE_ORDER[pressures["主胜"]] >= PRESSURE_ORDER[pressures["客胜"]]:
        return "顺分布"
    if gap <= -0.5 and PRESSURE_ORDER[pressures["客胜"]] >= PRESSURE_ORDER[pressures["主胜"]]:
        return "顺分布"
    if (gap >= 0.5 and PRESSURE_ORDER[pressures["客胜"]] > PRESSURE_ORDER[pressures["主胜"]]) or (
...
def build_original_distribution(
    *,
    ...
    """Build the pre-odds market distribution without reading odds."""
    ...
    dispersion = {
        direction: bool(pressures[direction] in {"强", "中"} or (pmap.get(direction) and pmap[direction].easy_to_receive))
    ...
    for direction in DIRECTIONS:
        ...
        if pull:
            confidence_sources.append(
                f"{direction}: 自然拉力={pull.strength or '未知'}；事实={pull.facts or '未确认'}；市场心理={pull.market_psychology or '未确认'}"
            )
    gap = _gap(strength)
    reasoning = [
        "原始分布只由基本面、广义实力和三项自然拉力生成；本阶段未读取赔率。",
        f"广义实力档位差={gap:g}；主胜/平局/客胜原始压力={pressures['主胜']}/{pressures['平局']}/{pressures['客胜']}。",
