...
def _friendly_defaults(m: dict[str, Any]) -> dict[str, Any]:
    """Fill stable match-rule fields for friendlies.

    GPT Actions may place "友谊赛、单回合、无加时点球" into competition or
    match_type text. The engine gate needs the normalized fields separately.
    """
    data = dict(m)
    text = " ".join(str(data.get(key) or "") for key in ("competition", "match_type", "stage"))
    is_friendly = any(key in text for key in ("友谊", "friendly", "Friendly", "热身"))
    if is_friendly:
        if not data.get("stage"):
            data["stage"] = data.get("match_type") or "国际友谊赛"
        if data.get("single_leg") is None:
            data["single_leg"] = True
        if not data.get("extra_time_or_penalties"):
            data["extra_time_or_penalties"] = "友谊赛无加时或点球规则，按90分钟常规时间审计"
    return data
...
def parse_raw_input(raw: dict[str, Any], *, diagnostics: list[PackageDiagnostic] | None = None, source_files: list[str] | None = None) -> LoadedInput:
    m = _friendly_defaults(raw["match"])
    match = MatchContext(
